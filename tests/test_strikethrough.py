...
* ``strikethrough_oracle.json`` - 209 curated cases WITH their rendered HTML.
  ...
  run against them.
* ``strikethrough_corpus.json`` - 5735 cases generated by
...
# Split up front: a test that enters a case and returns without asserting
# reads as a pass in the report, which is how a shrinking corpus would hide.
STRUCK_CASES = [c for c in CORPUS_CASES if c["struck"]]
CLEAN_CASES = [c for c in CORPUS_CASES if not c["struck"]]
...
class TestNoFalseNegatives:
    ...
    @pytest.mark.parametrize("case", STRUCK_CASES, ids=_ids(STRUCK_CASES))
    ...
    def test_known_misses_all_glue_two_regions_together(self):
        ...
        Every one of them puts two protected regions - a link macro, a bare URL,
        an image - on the same run of non-whitespace, and that is the whole
        reason they are exempt: Jira resolves the first and leaves the second as
        literal text, dashes and all, which written prose never does. If a miss
        is ever pinned that does NOT look like that, this fails and the
        exemption has to be argued rather than inherited.
