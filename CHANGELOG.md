...
- `find_strikethrough_spans()` implements the measured grammar and the lint reports only spans it finds; `escape_strikethrough()` repairs them, and `jira-comment.py add`/`edit` apply it before posting and report on stderr how many lines they changed, showing the first five (`\-` prints as a plain hyphen, so the reader sees no difference in prose). `--no-auto-escape` keeps the markup verbatim. Links, bare URLs and `!images!` are masked before the scan — the literal `/-/` in a GitLab merge-request URL is inert, and escaping it would break the link — while `{{monospace}}` is not, because Jira does parse text effects inside it. `{quote}` and `{panel}` are no longer treated as verbatim for the same reason; only `{code}` and `{noformat}` are
- The model is a deliberate **superset**: it may predict a span Jira would not draw (cost: one redundant `\-`, invisible in prose — the exception is two macros written against each other, where it can land inside a link target; three of the corpus's clean cases, all pinned) and must never miss one (cost: mangled text). It is also **not exact, and cannot be** — Jira substitutes autolinked issue keys before text effects run, so `OPS-899-x ... zu-` is struck on an instance where OPS-899 exists and clean on one where it does not. The same string, two renderings, decided by instance state that no source-level model can see

- A review of the above found four more defects in it, all of the same kind — a class the corpus alphabet did not contain, so nothing could have caught them. **U+00A0**: `str.isspace()` counts it and Jira does not, so `a -x<NBSP>- y` is struck through while the model called it clean; the delimiter whitespace class is ASCII-only now. NBSP arrives in anything pasted out of Word or Outlook. **A backslash-escaped bracket** was masked as a link, hiding the live dashes in `\[a -x- b\] z`. **A bare URL** was matched to the next whitespace, swallowing `{{...}}` and everything in it; measured, Jira's autolink stops at `{`, `}`, `]` and `|`, and does not fire at all after a pipe or an alphanumeric — while a square-bracketed link resolves anywhere, which also removed 19 of the 23 over-predictions. **The awk mirror** toggled its block flag on either tag, so `{noformat}` / `{code}` / `{noformat}` left it convinced it was still inside a block and it missed every span in the rest of the file; it tracks the open tag by name now, the way the Jira renderer and the Python side do
- The awk scan runs under a UTF-8 locale when the machine has one (`JIRA_SYNTAX_SCAN_LOCALE` overrides it, which is how the parity test still exercises the C-only fallback). Under `LC_ALL=C` a non-ASCII SYMBOL — `§`, `€`, `°`, `±` — is a run of high bytes and reads as a word character, so the opener boundary was lost and a real span missed. The fold list covers the punctuation that shows up in prose; the locale covers the rest
- The corpus alphabet gained `§`, U+00A0, `\[` and two tokens carrying U+00A0 INSIDE a region, which is why it grew to 11645 cases. Those three classes were each a live defect, and not one of them was reachable from the tokens the generator had. `--repin` recomputes the pinned lists from the recorded verdicts without calling Jira, so a model change no longer costs a full re-record - it was used four times while converging on this grammar

### Added

- The escape, lint and render-preview gates now run on every `--comment` and `--description` that writes wiki markup, not only on `jira-comment.py add`/`edit`. The other five — `jira-transition.py do --comment`, `jira-transition.py path --comment`, `jira-worklog.py add --comment`, and the `--description` of `jira-create.py issue` / `jira-issue.py update` — post the same markup through the same renderer and mangled it the same way, so the earlier fix for [#226](https://github.com/netresearch/jira-skill/issues/226) reached only two of the seven places the defect lives. `path` additionally had no mention gate. Each carries `--force`, `--no-auto-escape` and `--no-preflight`, spelled as on the comment path; a field written through `--fields-json` stays ungated by design, as do the two `--description` fields of `jira-version.py`, where it is unverified whether Jira renders them as wiki markup at all. Under `--dry-run` the escape and the lint run but the render call does not, so a preview prints the text a real run would post. `jira-create issue` is the one asymmetric case: the language lint gets the project key, the renderer gets none, because `POST /rest/api/1.0/render` resolves `issueKey` to a real issue and answers 404 for a project — so passing it would have printed "render preview unavailable" on every create
- `jira-comment.py add`/`edit` ask the Jira instance how it will render the comment before posting it, and refuse to post anything that comes back struck through. The local grammar is measured but it is still a model of the source text, and one class of defect sits outside it by construction: Jira substitutes autolinked issue keys before text effects run, so `{{OPS-899-Divergenzanalyse.pdf}}` renders struck through on an instance where OPS-899 exists and literally on one where it does not — same string, two renderings, decided by a row in the Jira database. Escaping the dash does not fix it, measured. `POST /rest/api/1.0/render` is the endpoint behind Jira's own preview button, so the verdict is the renderer's rather than ours. The check is advisory in every failure direction: no credentials, a Cloud instance (the endpoint is Server/DC only), a non-200 or a network error each print one warning and get out of the way. `--force` posts despite a struck preview, `--no-preflight` skips the call entirely, and `RenderVerdict.available` keeps "the renderer says this is clean" distinguishable from "nobody could ask it"
- `tests/fixtures/strikethrough_corpus.json` — 11645 generated cases with the verdict a live renderer returned for each, produced by `scripts/generate-strikethrough-corpus.py`. It exists because two hand-derived versions of this rule shipped wrong, and the second was wrong *while passing 168 hand-picked cases*: the case a human picks is a case they already thought of. The generator enumerates every ordered pair and triple over an alphabet of dash-relevant tokens instead. It found that a span body can contain a dash after all, which is why `journalctl -b -p crit; ... zu-` is one span. Against it the model has no unexplained misses; the 15 it does miss all put two macro-shaped tokens on one unbroken run of non-whitespace, which written prose does not do, and both those and the 12 over-predictions are pinned by name so neither list can grow unnoticed
- `tests/fixtures/strikethrough_oracle.json` — 209 curated cases with their full rendered HTML, re-recorded by `scripts/verify-render-oracle.py --live --record`. "Verified against Jira Server 9.12" is now a command rather than a sentence in a comment
- The parity test between `lib/markup.py` and the awk implementation in `validate-jira-syntax.sh` runs both over every single-line curated case in every locale on the machine, instead of twelve hand-picked ones. It caught two real divergences while being written: non-ASCII typographic quotes under `LC_ALL=C`, and a masked region reading as a word character in awk
...
- Hook: the injected issue-reference hint lists `jira-issue.py update`, so setting a field no longer means finding it in `SKILL.md` first
