...
- The escape, lint and render-preview gates now run on every command that writes wiki markup, not only on `jira-comment.py add`/`edit`. The other four surfaces — `jira-transition.py do --comment`, `jira-worklog.py add --comment`, and the `--description` of `jira-create.py issue` / `jira-issue.py update` — post the same markup through the same renderer and mangled it the same way, so the fix for [#226](https://github.com/netresearch/jira-skill/issues/226) reached two of the six places the defect lives. Each carries `--force`, `--no-auto-escape` and `--no-preflight`, spelled as on the comment path. `jira-create issue` is the one asymmetric case: the language lint gets the project key, the renderer gets none, because `POST /rest/api/1.0/render` resolves `issueKey` to a real issue and answers 404 for a project — so passing it would have printed "render preview unavailable" on every create
