...
**`[~mention]` in comments needs the canonical username, not a guess.** The mention token resolves on the account's `name`/`key` — which can differ from **both** the display name and the email local-part (renamed accounts are common: e.g. display "Jane Doe", email `jane.doe@…`, but `name=jane.smith` after a rename). Guessing from the display name or email silently produces a non-notifying mention. `jira-comment add/edit` therefore verifies every `[~username]` at post time and aborts with suggestions on a miss (`--no-verify-mentions` skips). For ticket participants no lookup is needed at all: `jira-issue get/work` and `jira-comment list` print the technical username in parentheses next to each display name — copy it into `[~<name>]`. For anyone else, `jira-user.py search "<display name>"` resolves it. If someone says "your mention pinged the wrong/no user", this is why. (Jira **Cloud** uses `[~accountId:<accountId>]` instead of `[~username]`; resolve the `accountId` the same way and use that form.)
