...
An earlier version of this section claimed the boxes need a browser session. That conclusion came from probing the WRONG endpoint: `/rest/inline-tasks/1.0/task/<id>` does 302-redirect PATs to `login.jsp` — but that is a different plugin's route, not the one these checkboxes use (re-confirmed 2026-08-12: inline-tasks 302s while tasklist answers 204 with the same PAT). If the `netresearch-jira` skill is installed, use its ready-made wrapper instead of raw curl: `uv run <netresearch-jira>/scripts/jira-task.py list KEY` / `check KEY <task-id>` (see that skill's `references/_global/task-checklists.md` for the curl gotchas: JSON body → 415, query param ignored, form body is authoritative).
