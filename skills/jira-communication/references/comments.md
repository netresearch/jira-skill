...
The three gates described here — escape, lint, render preview — run on every command that writes wiki markup, not only on comments: `jira-comment add`/`edit`, `jira-transition do --comment`, `jira-worklog add --comment`, and the `--description` of `jira-create issue` / `jira-issue update`. The same six as the mention gate below, and the three flags (`--force`, `--no-auto-escape`, `--no-preflight`) are spelled the same on each. The text below says "the body"; a description is checked identically.
