...
**CRITICAL**: This skill exists to enforce correct Jira syntax. Never use Markdown syntax in Jira content.

### Common Syntax Patterns

**Headings**: Use `h1.` through `h6.` with space
```
h2. Main Section
h3. Subsection
```
...
**Text Formatting**:
- Bold: `*text*` (NOT `**text**`)
- Italic: `_text_` (NOT `_text_`)
- Monospace/code: `{{text}}` (NOT `` `text` ``)

**Lists**:
- Bullets: `* item` with `**` for nesting
- Numbers: `# item` with `##` for nesting

**Code Blocks**: Use `{code:language}` NOT markdown code fences
```
{code:java}
public void example() {}
{code}
...
- Issues: `[PROJ-123]`
- Users: `[~username]`
- External: `[Label|http://url]` (NOT `[Label](url)`)
...
**Panels** (for highlighting important content):
```
{panel:title=Important|bgColor=#FFEBE9}
Content here
{panel}
```
...
Marketplace sync appends commit date: `1.0.0-20251107`

## Common Patterns and Best Practices

### Creating Issues with Templates
1. Use appropriate template (bug-report or feature-request)
2. Fill in all required sections
3. Ensure proper heading hierarchy (h2. → h3. → h4.)
4. Add code blocks with language specification
5. Link related issues with `[PROJ-XXX]` format
6. Mention stakeholders with `[~username]`
...
# Assigned to current user, updated last week
...
### Structured Comments
Always use proper structure for comments:
- h3. heading for comment topic
- Bulleted or numbered lists for details
...
Per user's global instructions, when working with Jira in production:
- Ticket descriptions, summaries, and comments should be in German
- Keep professional tone in Jira content
- Only add comments when explicitly instructed during transitions
